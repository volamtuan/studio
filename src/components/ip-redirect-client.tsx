...
import type { IpLinkConfig } from '@/app/actions/ip-links';

interface IpRedirectClientProps {
  config: IpLinkConfig;
}

export function IpRedirectClient({ config }: IpRedirectClientProps) {
  const { id, redirectUrl } = config;
  const [statusText, setStatusText] = useState('Đang xác minh và chuyển hướng...');
